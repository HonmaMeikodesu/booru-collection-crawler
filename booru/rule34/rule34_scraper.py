...
"""
Rule34 Web Scraper
A command-line tool for downloading images and metadata from Rule34
"""
...
class Rule34Scraper:
    ...
    def _validate_proxy(self):
        """Test proxy connection to Rule34"""
        try:
    ...
    def _build_search_url(self, tags: str, page: int = 1) -> str:
        ...
        if page == 1:
            ...
            # Calculate pid (pagination ID)
            pid = (page - 1) * 42  # Rule34 shows 42 posts per page
            return f"{BASE_URL}/index.php?page=post&s=list&tags={encoded_tags}&pid={pid}"
    
    def get_last_page_number(self, tags: str) -> int:
        """
        Get last page number by following next links until exhaustion
        Returns: total number of pages
        """
        url = self._build_search_url(tags, 1)
        ...
        # Look for next link: <a href="..." alt="next">
        next_link = paginator.select_one('a[alt="next"]')
        if not next_link:
            # Only one page
            return 1
        
        # Find last page link (the one with ">>" or "last page")
        last_page_link = paginator.select_one('a[alt="last page"]')
        if last_page_link and last_page_link.get('href'):
            ...
            if match:
                ...
                # Calculate page number from pid (pid = (page - 1) * 42)
                total_pages = (pid // 42) + 1
                return total_pages
        
        # Fallback: count all numbered page links
    ...
    def get_post_ids_from_page(self, tags: str, page: int) -> List[int]:
        ...
        url = self._build_search_url(tags, page)
        ...
        post_links = soup.select('#post-list .image-list > span > a')
        
        for link in post_links:
            post_id_attr = link.get('id')
            if post_id_attr:
                # Extract post ID from id like "p15795581"
                match = re.search(r'p(\d+)', post_id_attr)
                if match:
    ...
    def get_post_details(self, post_id: int) -> Tuple[Optional[str], Dict[str, List[str]]]:
        ...
        # Extract image URL - find anchor with text "Original image"
        image_url = None
        all_links = soup.select('li a')
        for link in all_links:
            if link.get_text(strip=True) == "Original image":
        ...
        if tag_sidebar:
            ...
            for item in items:
                ...
                if h6:
                    ...
                    # This is a tag item
                    if current_category and 'tag' in item.get('class', []):
                        # Find the tag link (second or third <a> in the li)
                        tag_links = item.find_all('a')
                        for tag_link in tag_links:
                            href = tag_link.get('href', '')
                            if 's=list&tags=' in href:
                                # Extract tag name from href
                                match = re.search(r'tags=([^&]+)', href)
                                if match:
...
def mode_new(args):
    ...
    logger.info(f"Rule34 Scraper v{VERSION}")
    ...
    try:
        # Get total pages
        total_pages = scraper.get_last_page_number(args.tags)
        ...
        logger.error(f'  python rule34_scraper.py --mode resume --task-path "{task_manager.task_folder}"')
...
def download_posts(scraper: Rule34Scraper, task_manager: TaskManager, 
...
def sync_posts(scraper: Rule34Scraper, task_manager: TaskManager,
    ...
    total_pages = scraper.get_last_page_number(tags)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Rule34 Web Scraper - Download images and metadata from Rule34',
