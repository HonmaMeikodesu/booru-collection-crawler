...
"""
TBIB Web Scraper
A command-line tool for downloading images and metadata from The Big ImageBoard (TBIB)
"""
...
class TbibScraper:
    ...
    def get_all_post_ids(self, tags: str) -> List[int]:
        """
        Get all post IDs for search tags by traversing pagination
    ...
    def _build_search_url(self, tags: str) -> str:
        """Build search URL from tags"""
        # Split tags and URL-encode them
        tag_list = tags.split()
    ...
    def _extract_post_ids_from_page(self, soup: BeautifulSoup) -> List[int]:
        ...
        # Find all post preview links using the specified selector
        # CSS: #post-list .content div span > a
        post_list = soup.select_one('#post-list .content')
        if not post_list:
            return post_ids
        
        post_links = post_list.select('div span > a')
        
        for link in post_links:
            # Get id attribute (format: p{post_id})
            post_id_attr = link.get('id')
            if post_id_attr and post_id_attr.startswith('p'):
                try:
                    post_id = int(post_id_attr[1:])  # Remove 'p' prefix
    ...
    def get_post_details(self, post_id: int) -> Tuple[Optional[str], Dict[str, List[str]]]:
        ...
        # Extract image URL - find anchor with "Original image" text
        image_url = None
        ...
        if not image_url:
            logger.error(f"Post {post_id}: Original image link not found")
            raise ValueError(f"Original image link not found for post {post_id}")
        ...
        if tag_sidebar:
            ...
            for element in tag_sidebar.find_all(['h6', 'li']):
                if element.name == 'h6':
                    # Category header
                    category_name = element.get_text(strip=True).lower()
                    ...
                    # Tag item - check for tag-type class or tag class
                    element_classes = element.get('class', [])
                    # Meta tags use 'tag-type-metadata' class, others use 'tag-type-{category}'
                    has_tag_class = any('tag-type-' in cls or cls == 'tag' for cls in element_classes)
                    if has_tag_class:
...
def mode_new(args):
    ...
    logger.info(f"TBIB Scraper v{VERSION}")
    ...
    try:
        ...
        logger.error(f'  python tbib_scraper.py --mode resume --task-path "{task_manager.task_folder}"')
...
def download_posts(scraper: TbibScraper, task_manager: TaskManager, 
...
def sync_posts(scraper: TbibScraper, task_manager: TaskManager,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='TBIB Web Scraper - Download images and metadata from The Big ImageBoard (TBIB)',
