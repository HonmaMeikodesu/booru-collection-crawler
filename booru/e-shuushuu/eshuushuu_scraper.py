#!/usr/bin/env python3
"""
E-Shuushuu Web Scraper
A command-line tool for downloading images and metadata from E-Shuushuu
"""
...
class ImageNotFoundError(Exception):
    """Raised when image link not found"""
    pass


class EShuushuuScraper:
    """Main scraper class for E-Shuushuu"""
    ...
    def _validate_proxy(self):
        """Test proxy connection to E-Shuushuu"""
        try:
    ...
    def _build_search_url(self, tag_id: str, page: int = 1) -> str:
        """Build search URL from tag ID and page number"""
        if page == 1:
            return f"{BASE_URL}/search/results/?tags={tag_id}"
        else:
            return f"{BASE_URL}/search/results/?page={page}&tags={tag_id}"
    
    def get_all_post_ids(self, tag_id: str) -> List[int]:
        """
        Get all post IDs by following pagination until exhaustion
        Returns list of post IDs
        """
        all_post_ids = []
        current_url = self._build_search_url(tag_id)
        page_num = 1
        ...
        while current_url:
            ...
            if pagination:
                ...
                if next_link and next_link.get('href'):
                    ...
                    # The href contains query params like "?page=2&tags=76604"
                    # We need to append to the search results path
                    next_href = next_link['href']
                    current_url = f"{BASE_URL}/search/results/{next_href}"
                    page_num += 1
                else:
                    # No more pages
                    current_url = None
            else:
                # No paginator, only one page
                current_url = None
    ...
    def _extract_post_ids_from_page(self, soup: BeautifulSoup) -> List[int]:
        ...
        # Find all post containers using .image_thread.display
        post_containers = soup.select('.image_thread.display')
        
        for container in post_containers:
            # Get id attribute (format: i{post_id})
            post_id_attr = container.get('id')
            if post_id_attr and post_id_attr.startswith('i'):
                try:
                    post_id = int(post_id_attr[1:])  # Remove 'i' prefix
                    post_ids.append(post_id)
                except ValueError:
                    logger.warning(f"Invalid post ID format: {post_id_attr}")
                    continue
    ...
    def get_post_details(self, post_id: int) -> Tuple[Optional[str], Dict[str, List[str]]]:
        """
        Get image URL and tags for a post from the search results page
        Returns: (image_url, tags_dict)
        """
        # Note: E-Shuushuu provides image URL directly in search results
        # We need to fetch the search page and find the specific post
        # For simplicity, we'll use the post detail page approach
        url = f"{BASE_URL}/image/{post_id}/"
        ...
        # Extract image URL from .thumb_image anchor
        image_url = None
        thumb_image = soup.select_one('.thumb_image')
        if thumb_image and thumb_image.get('href'):
            image_url = urljoin(BASE_URL, thumb_image['href'])
        
        if not image_url:
            raise ImageNotFoundError(f"Image link not found for post {post_id}")
        ...
        # Find meta section with tags
        meta_section = soup.select_one('.meta')
        if meta_section:
            ...
            for dt in dt_elements:
                category_text = dt.get_text(strip=True).lower()
                
                # Map category names
                if category_text == 'tags:':
                    current_category = 'tag'
                elif category_text == 'source:':
                    current_category = 'source'
                elif category_text == 'characters:':
                    current_category = 'character'
                elif category_text == 'artist:':
                ...
                if dd and current_category and dd.get('class') and 'quicktag' in dd.get('class'):
                    # Extract tags from span.tag elements
...
class TaskManager:
    ...
    def sanitize_tag_id(tag_id: str) -> str:
        """Sanitize tag ID for folder name"""
        # Use tag_ prefix with the tag ID
        sanitized = f"tag_{tag_id}"
        # Remove any special characters except underscore and hyphen
        sanitized = re.sub(r'[^a-zA-Z0-9_-]', '', sanitized)
        return sanitized
    
    @staticmethod
    def create_task_folder(storage_path: Path, tag_id: str) -> 'TaskManager':
        ...
        # Create task folder
        folder_name = TaskManager.sanitize_tag_id(tag_id)
...
def mode_new(args):
    ...
    logger.info(f"E-Shuushuu Scraper v{VERSION}")
    logger.info(f"Mode: new")
    logger.info(f"Tag ID: {args.tag_id}")
    ...
    # Create task folder
    task_manager = TaskManager.create_task_folder(Path(args.storage_path), args.tag_id)
    
    # Initialize metadata
    metadata = {
        'search_tag_id': args.tag_id,
    ...
    try:
        ...
        all_post_ids = scraper.get_all_post_ids(args.tag_id)
        ...
        logger.error(f'  python eshuushuu_scraper.py --mode resume --task-path "{task_manager.task_folder}"')
...
def mode_resume(args):
    ...
    logger.info(f"Tag ID: {metadata['search_tag_id']}")
...
def download_posts(scraper: EShuushuuScraper, task_manager: TaskManager, 
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='E-Shuushuu Web Scraper - Download images and metadata from E-Shuushuu',
    ...
    parser.add_argument('--tag-id', help='Tag ID to search (required for new mode)')
    ...
    if args.mode == 'new':
        if not args.tag_id or not args.storage_path:
            logger.error("--tag-id and --storage-path are required for new mode")
