#!/usr/bin/env python3
"""
Safebooru Web Scraper
A command-line tool for downloading images and metadata from Safebooru
"""
...
class SafebooruScraper:
    """Main scraper class for Safebooru"""
    ...
    def _validate_proxy(self):
        """Test proxy connection to Safebooru"""
        try:
    ...
    def _extract_post_ids_from_page(self, soup: BeautifulSoup) -> List[int]:
        ...
        # Find all post preview links: #post-list .image-list a
        post_links = soup.select('#post-list .image-list a')
        if not post_links:
            return post_ids
    ...
    def get_post_details(self, post_id: int) -> Tuple[Optional[str], Dict[str, List[str]]]:
        ...
        if tag_sidebar:
            ...
            for li in tag_sidebar.find_all('li'):
                # Check if this is a category header (h6 tag)
                h6 = li.find('h6')
                if h6:
                    ...
                    # Check if this li has tag-type class
                    is_tag_item = any(cls.startswith('tag-type-') for cls in tag_classes)
                    if is_tag_item:
                        # Find the tag link (the one that points to post list)
...
def download_posts(scraper: SafebooruScraper, task_manager: TaskManager, 
...
def sync_posts(scraper: SafebooruScraper, task_manager: TaskManager,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Safebooru Web Scraper - Download images and metadata from Safebooru',
