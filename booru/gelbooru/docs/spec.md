你需要给图片存档网站`Gelbooru`开发一个爬虫程序（使用Python），以下是用户使用的时序流程：
1. 用户输入一段单数或复数的tags（例如`honma_meiko`, `honma_meiko ano_hi_mita_hana_no_namae_wo_bokutachi_wa_mada_shiranai.`）
2. 拼装出搜索tags的url: 例如`https://gelbooru.com/index.php?page=post&s=list&tags=honma_meiko`, `https://gelbooru.com/index.php?page=post&s=list&tags=honma_meiko+ano_hi_mita_hana_no_namae_wo_bokutachi_wa_mada_shiranai.`。**可见根据用户输入的tags构建url时，需要对复数的tags进行`+`拼接，同时每个tag需要进行URL编码**
...
4. 从第1页到最后一页，获取每一个post的id(`document.querySelectorAll(".thumbnail-container article > a")`可以拿到当前页下的所有post)
5. 拼装出获取post内容的url：例如`https://gelbooru.com/index.php?page=post&s=view&id=13022516`
6. 分析返回的页面DOM，参考[post.html](./post.html)，获取post的图片资源以及tags并存储在本地。
   ...
   2. tags在`#tag-list`的`li`元素下，分为`Copyright`, `Character`, `Artist`, `Metadata`, `Tag`五个类别
