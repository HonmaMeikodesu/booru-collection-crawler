...
A command-line tool for downloading images and metadata from Gelbooru
"""
...
class GelbooruScraper:
    """Main scraper class for Gelbooru"""
    ...
    def _validate_proxy(self):
        """Test proxy connection to Gelbooru"""
        try:
    ...
    def _extract_post_ids_from_page(self, soup: BeautifulSoup) -> List[int]:
        ...
        # Find all post preview links using .thumbnail-container article > a
        thumbnail_container = soup.select_one('.thumbnail-container')
        if not thumbnail_container:
            return post_ids
        
        post_links = thumbnail_container.select('article > a')
...
def mode_new(args):
    ...
    logger.info(f"Gelbooru Scraper v{VERSION}")
    ...
    try:
        ...
        logger.error(f'  python gelbooru_scraper.py --mode resume --task-path "{task_manager.task_folder}"')
...
def download_posts(scraper: GelbooruScraper, task_manager: TaskManager, 
...
def sync_posts(scraper: GelbooruScraper, task_manager: TaskManager,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Gelbooru Web Scraper - Download images and metadata from Gelbooru',
