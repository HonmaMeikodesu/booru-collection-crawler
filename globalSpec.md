...
- User input the search condition and a local file path where to store the resource
...
### When restart the already completed task
images on the remote server are constantly updated with time passing by, so the post list result tomorrow will be different from it today. When restart the already completed task, update the post list by latest server data, and **sync** the resource between remote and local(download new posts, old posts remain intact). 
...
- Dont write any GUI logic, command-line execution is suffient.
- Support proxy config in case some websites are restricted within some areas